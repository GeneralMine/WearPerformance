<script>
	...
	export let primaryColor = '#25be20';
	...
	// Do Animation on change
	$: doAnimation(checked);
	$: applyGroupChecked(selection);
	function applyGroupChecked(sel) {
		if (id !== '') {
			checked = sel.includes(id);
			doAnimation(checked);
		}
	}
	function doAnimation(ch) {
		if (!canChange) return false;
		if (animationChecked === ch) {
			return;
		}
		canChange = false;
		if (!ch) {
...
>
	<input id={labelId} type="checkbox" on:change={toggleChecked} {name} />
...
<style>
	...
	.checkbox__border {
		...
		transform: translate(calc(var(--checkbox-border-width) * -1), var(--checkbox-border-width))
			rotate(90deg);
